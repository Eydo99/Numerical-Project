...
if DD:
    print("matrix is diagonally dominant => Gauss sediel will to converge.")
    A = newA
else:
    print("matrix is not diagonally dominant => Gauss sediel may not converge.")
...
for i in range(It):
    ...
    for row in range(len(A)):
        ...
        Intial[row]=(C[row]+computation)*(1/A[row][row])


    Intial = [float(f"{val:.{sig}g}") for val in Intial]


    errors = []
    for j in range(n):
        if Intial[j] != 0:
            Ea=abs((Intial[j]-Old[j])/Intial[j])*100
        else:
            Ea = 0
        errors.append(round(Ea, 6))

    for j in range(n):    
       print(X[j]+"="+str(Intial[j])+"   (error = "+str(errors[j])+"%)")
       print()
    ...
    if stop and (i+1)!=It:
      print("Stopping early: all relative errors < margin.\n")
      break
    
    


print("Gauss Sediel End")
